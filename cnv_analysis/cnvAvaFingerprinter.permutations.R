.libPaths(c("/mnt/work1/users/pughlab/bin/r_lib/library/testLib",
            .libPaths(), "/mnt/work1/users/pughlab/bin/r_lib/library/3.1"))
###############################################################
#
...
existing.rdata.dir <- cn.dir;  # Set the directory if create.df is set to 0, else default "./rdata" will be used
if(length(args) >= 3){
...
generateVarMatrix <- function(uniq.cl.x=NULL, max.sd = 2, ...){
  ...
  compareCn <- function(cn1, cn2, median.norm=0, quant.norm=0){
    if(median.norm==1){
  ...
  if(length(uniq.cl.x) > 1){
    ...
    for(each.combn in c(1:dim(uniq.combn)[2])){
      ...
      combn.sd <- compareCn(total.rdata.list[[id1]][,seg.header], 
                            ...
                            median.norm=median.norm, quant.norm=quant.norm)
...
compareCnProfiles <- function(cn.binned.df1, cn.binned.df2, seg.col.name, num.bins, mdiff, 
                              min.thresh=1.5, max.thresh=3, median.norm=0, quant.norm=0){
...
getSsdScoreTally <- function(cn.binned.df1, cn.binned.df2, seg.col.name, num.bins, 
                             mdiff, median.norm=0, quant.norm=0){
  cn1 <- cn.binned.df1[,seg.col.name]
  cn2 <- cn.binned.df2[,seg.col.name]
...
plotSd <- function(file.l=NULL, comp.all=FALSE, total.rdata.list=total.rdata.list, 
                   seg.header=NULL, plots.path=NULL, median.norm=0, quant.norm=0){
  require(pastecs)    
  require(corrgram)
  require(scales)
  ...
  for(file.a in file.l){
    ...
    for(file.b in comp.against){
      ...
      if(!file.a == file.b){
        ...
        png(file.path(plots.path, file.a, gsub(".Rdata", ".1.png", file.b)), 
            width=2000, height=1750, res=45)
        ...
        png(file.path(plots.path, file.a, gsub(".Rdata", ".2.png", file.b)), 
            width=2000, height=1750, res=45)
        diff.cn <- cn2-cn1
...
generateEasyClNames <- function(each.cl.name){
  ...
  # Get the sub it derived from and the cell id
  sub.regex <- regexpr("sub[0-9]+\\.", each.cl.name, perl=TRUE)
  sub.id <- regmatches(each.cl.name, sub.regex)
  sub.id <- gsub(".$", "", sub.id)
  cl.regex <- regexpr("\\.(.+)$", each.cl.name, perl=TRUE)
  cl.id <- regmatches(each.cl.name, cl.regex)
  each.cl.name <- gsub("^.", "", cl.id)
  ...
  return(paste(sub.id, new.cl.name[1], sep=""))
}
...
if(process.data == 1){  
  ...
  for(seg.header in header.list){
    ...
    if(create.df == 1){
      # R set up embarrassingly parallel computing
      ...
      if(existing.rdata.dir != ""){
        rdata.path <- file.path(existing.rdata.dir, seg.header, "input")
      }
    }
    ...
    if(!file.exists(file.path(output.dir, seg.header, "sd_calc", "ssd.matrix.R"))){
      ...
      for(rdata.f1 in names(total.rdata.list)){
        ...
        subset.comp <- lapply(total.rdata.list[c(val:length(total.rdata.list))], function(x) getSsdScoreTally(total.rdata.list[[rdata.f1]], x, 
                                                                                                              seg.header, total.bins, max.diff, 
                                                                                                              median.norm=0, quant.norm=0))
      ...
      # Add proper names for the SSD matrix
      ssd.score.df <- relabelMatrix(ssd.score.df, gsub(paste(".", seg.header, ".Rdata", sep=""), ".CEL", colnames(ssd.score.df)))
      ssd.score.df <- relabelMatrix(ssd.score.df, 
                                    unlist(lapply(colnames(ssd.score.df), function(x) generateEasyClNames(x))))
    ...
    # Generates the CN-bin 
    sub.rm.names <- gsub("^sub[0-9]+\\.", "", names(total.rdata.list))
    sub.rm.names <- gsub("\\.hscr.+$", "", sub.rm.names)
    matched.repeats <- lapply(levels(factor(sub.rm.names)), function(x) {
                              names(total.rdata.list)[which(sub.rm.names %in% x)]})
    names(matched.repeats) <- levels(factor(sub.rm.names))
    median.norm <- 0
    quant.norm <- 1
    # Generates the squared-diff matrices by genomic bins and adjusts by Std.dev weight
    all.sd.mat <- lapply(matched.repeats, function(uniq.cl.x) generateVarMatrix(uniq.cl.x))
#     all.adj.sd <- lapply(u266, function(ds.x){
#       all.sd.mat <- ds.x[['sd']]
#       lapply(all.sd.mat, function(ccl.x){
#         apply(ccl.x$sdiff, 2, function(sdiff.col) return(sdiff.col * ccl.x$stdev))
#       })
#     })
    ...
    if(length(plot.list) > 0){
      plotSd(file.l=plot.list, comp.all=FALSE, total.rdata.list=total.rdata.list,
             seg.header=seg.header, plots.path=plots.path, median.norm=0, quant.norm=1)
    }
    ...
    save(total.matrix, file=file.path(data.matrix.dir, out.total.rdata))
    
    spear.corr.matrix <- cor(total.matrix, method = "spearman")
    spear.corr.matrix <- relabelMatrix(spear.corr.matrix, gsub(paste(".", seg.header, ".Rdata", sep=""), ".CEL", colnames(spear.corr.matrix)))
