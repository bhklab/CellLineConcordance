...
load("~/git/snp_fingerprint/environments/plotPairwiseSnp.v2.Rdata")

setwd(mismatch.dir)
outdir <- file.path(mismatch.dir, "output")
threshold <- 0.8
max.length <- 10
hscr <- 'hscra2'
...
if(!is.null(alt.seg.dir)){
  ...
  # list.x <- list(list("i"='MDA−MB−468', "c1"='GDSC',
  #                     "j"='MDA-MB-468 (IVCC)', "c2"='GNE'))
  setwd(mismatch.dir)
...
# 
# 
# # ## Estimate the degree of chromosomal instability based on the number of chromosome segments
# # cl_cin <- apply(hscr.mat.a12, 2, function(x, cutoff_thresh){
# #   rle_x <- rle(x)
# #   length(which(rle_x$lengths > cutoff_thresh))
# # }, cutoff_thresh=10)
# 
# 
# transformed.list <- list(list("i"='KNS-81-FD', "c1"="GDSC",
#                               "j"='KNS-81-FD',"c2"="CCLE"))
# transformed.diff <- runAndPlotSummary(transformed.list, "transformed")
# names(transformed.diff) <- sapply(transformed.list, getDsName)
# 
# drift.list <- list(list("i"='COLO-320-HSR', "c1"="GDSC",
#                         "j"='COLO-320-HSR',"c2"="CCLE"),
#                    list("i"='REH', "c1"="GDSC",
#                         "j"='REH',"c2"="CCLE"),
#                    list("i"='NCI-H23', "c1"="GDSC",
#                         "j"='NCI-H23',"c2"="CCLE"))
# discordant.list <- list(list("i"='AU565', "c1"="Pfizer",
#                              "j"='SK-BR-3',"c2"="Pfizer"),
#                         list("i"='HPAC', "c1"="CCLE",
#                              "j"='KCI-MOH1',"c2"="CCLE"),
#                         list("i"='OCI-LY10', "c1"="CCLE",
...
# c1 <- 'ccle'
# c2 <- 'ccle'
# adj.diff.data <- plotAllHscr()
# cytoband.diff <- processCytobandDiff(cytoband.diff)
# diff.list[[paste(paste(c1, i, sep="-"), 
#                  paste(c2, j, sep="-"), sep=".")]] <- cytoband.diff
# 
# 
# drift.diff <- runAndPlotSummary(drift.list, "drift")
# names(drift.diff) <- sapply(drift.list, getDsName)
# 
# discordant.diff <- runAndPlotSummary(discordant.list, "discordant")
# names(discordant.diff) <- sapply(discordant.list, getDsName)
# 
# transformed.diff <- runAndPlotSummary(transformed.list, "transformed")
# names(transformed.diff) <- sapply(transformed.list, getDsName)
# 
# delta_goi <- list()
# # Low CNV Correlation
# cl.list <- c('Ishikawa (Heraklio) 02 ER-', 'MOG-G-CCM', 'NAMALWA', 'NCI-H23', 'U251', 'Set-2', 'Hs 940.T', 'SCC-15', 'REH', 
#              'HuP-T4', 'MCF7', 'MDA-MB-415', 'NCI-H1568', 'Mewo', 'HCC-78')
# # High CNV Correlation
# cl.list <- c('SW620', 'CAL-148', 'P31-FUJ', 'MIA-PaCa-2', 'COLO-668', 'TOV-21G', 'HL-60', 'NCI-H1869', 'NCI-SNU-1', 'NMC-G1', 
#              'NCI-H2196', 'SU-DHL-8', 'G-402', 'CL-40', 'MV-4-11')
# # Mid CNV Correlation
# cl.list <- c('697', '786-0', 'CAKI-1', 'CAPAN-1', 'CML-T1', 'COLO-320-HSR', 'COLO-680N', 'COLO-783', 'DB', 'G-361', 'GA-10', 
#              'HCC-33', 'HCC-78', 'HCC2218', 'Hs 939.T', 'HuP-T3', 'HuP-T4', 'JHOS-2', 'JHOS-4', 'L-363', 'LK-2', 'LOXIMVI', 
#              'MCF7', 'MDA-MB-157', 'MDA-MB-415', 'Mewo', 'MOLT-16', 'NB1', 'NCI-H1568', 'NCI-H1755', 'NCI-H2009', 'NCI-H2066', 
#              'NCI-H2087', 'NCI-H2122', 'NCI-H2141', 'NCI-H522', 'OAW-28', 'OE21', 'OUMS-23', 'OVCAR-3', 'OVTOKO', 'PC-3', 
#              'PE/CA-PJ15', 'PSN1', 'SCaBER', 'SK-MEL-24', 'SK-MEL-5', 'T98G', 'TALL-1', 'TE-15', 'U-2-OS', 'U-266', 'UACC-812', 
#              'UACC-893')
# cl.list <- c('HUH-6-Clone5')
# for(i in cl.list){
#   goi.list[[which(i == cl.list)]] <- list("i"=i, "c1"="CCLE",
#                                        "j"=i, "c2"="GDSC")
# }
# goi.diff <- runAndPlotSummary(goi.list, "lowCnvConc")
# names(goi.diff) <- sapply(goi.list, getDsName)
# delta_goi[['midCnv']] <- goi.diff
# 
# 
# goi.list <- list(list("i"='SK-BR-3', "c1"="Pfizer",
#                       "j"='AU565',"c2"="CCLE"),
#                  list("i"='SK-BR-3', "c1"="CCLE",
#                       "j"='AU565',"c2"="Pfizer"),
#                  list("i"='SK-BR-3', "c1"="CCLE",
#                       "j"='AU565',"c2"="CCLE"),
#                  list("i"='SK-BR-3', "c1"="Pfizer",
#                       "j"='AU565',"c2"="Pfizer"),
#                  list("i"='SK-BR-3', "c1"="CCLE",
#                       "j"='SK-BR-3',"c2"="Pfizer"),
#                  list("i"='AU565', "c1"="CCLE",
#                       "j"='AU565',"c2"="Pfizer"))
# goi.list <- list(list("i"='HUH-6-clone5', "c1"="GDSC",
#                       "j"='HUH-6-clone5',"c2"="CGP"),
#                  list("i"='BT-549', "c1"="CGP",
#                       "j"='BT-549',"c2"="Pfizer"))
# goi.diff <- runAndPlotSummary(goi.list, "goi")
# names(goi.diff) <- sapply(goi.list, getDsName)
# 
# #skbr3.au565.goi.diff <- goi.diff
# 
# getStats <- function(x.name, x.list){
#   x.list.single <- x.list[[x.name]]
#   print(x.name)
#   print(x.list.single[['ccl.data']])
#   NULL
# }
# lapply(names(drift.diff), function(x) getStats(x, drift.diff))
# 
# 
# # drift.diff <- lapply(drift.list, runListFun)
# # names(drift.diff) <- sapply(drift.list, getDsName)
# # 
# # discordant.diff <- lapply(discordant.list, runListFun)
# # names(discordant.diff) <- sapply(discordant.list, getDsName)
# # 
# # transformed.diff <- lapply(transformed.list, runListFun)
# # names(transformed.diff) <- sapply(transformed.list, getDsName)
# 
# 
# 
# pdf(file.path("cnv_summary", "transformed.pdf"), height = 2.5)
# lapply(transformed.diff, function(diff.list) plotCytobandDiff(diff.list, chr.df))
# dev.off()
# 
# 
# 
...
# 
#   
# getMmDf(i = i, j = j, c1 = c1, c2 = c2, hscr.mat=hscr.mat.a12, stdev.list=stdev.list.a12, hscr='hscra12')
# # 'MDA-MB-436','T47D', 'ZR751'
# for(i in c('Hs-578-T','SK-BR-3')){
#   getMmDf(i, c1, c2)
# }
# 
# l1 <- list.files("cnv_plots", pattern="COLO-320.*png")
# 
# h1 <- rasterGrob(readPNG(file.path("cnv_plots", l1[2]), native = FALSE), interpolate=FALSE)
# h2 <- rasterGrob(readPNG(file.path("cnv_plots", l1[3]), native = FALSE), interpolate=FALSE)
# h12 <- rasterGrob(readPNG(file.path("cnv_plots", l1[1]), native = FALSE), interpolate=FALSE)
#            
