###############################################################
#
#                 VCF To Affy6 Genotype Converter
#
# Author: Rene Quevedo
# Date Created: August 25-2016
###############################################################
# Function: Takes a VCF and converts the genotype format 0/0,
...
#   -HaplotypeCaller
#
#   Usage: Rscript vcfToAffyGT [vcf] [affyAnno] [outfile]
###############################################################
...
vcf.df <- read.csv(vcfFile, sep="\t", header=TRUE, comment.char = "#",
...
colnames(vcf.affy.df) <- c('probeset_id', gsub(".vcf", "", basename(vcfFile)))
